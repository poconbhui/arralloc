...
int main(void){
    ...
    /* Allocate the array */
    array_1d = (int*) arralloc(sizeof(int), 1, 500);
    ...
    /* Allocate the array */
    array_2d = (int**) arralloc(sizeof(int), 2, 5,100);
    ...
    for(j=0; j<100; ++j) {
        ...
        if((&array_2d[0][0])[i*100+j] != i*100 + j) {
            fprintf(
                ...
                i*100+j, (&array_2d[0][0])[i*100+j], i*100+j
            );
    ...
    /* Allocate the array */
    array_3d = (int***) arralloc(sizeof(int), 3, 5,100,88);
    ...
    for(k=0; k<88; ++k) {
        ...
        if(array_3d[i][j][k] != i*100*88 + j*88 + k) {
            fprintf(
                ...
                i, j, k, array_3d[i][j][k], i*100*88 +j*88 + k
            );
        ...
        if((&array_3d[0][0][0])[i*100*88 +j*88 + k] != i*100*88 + j*88 + k) {
            fprintf(
                ...
                i*100*88 +j*88 + k,
                (&array_3d[0][0][0])[i*100*88 +j*88 + k],
                i*100*88 +j*88 + k
            );
    ...
    /* Allocate the array */
    array_4d = (int****) arralloc(sizeof(int), 4, 5,100,88,7);
